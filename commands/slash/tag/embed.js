...
await int.deferReply({
    ...
    const tagIndex = userData.tags.findIndex(tag => tag.name === tagName) > 0
    ...
    if (!removeEmbedIndex && tagData.data?.embeds?.length >= 25) return int.editReply({
        content: "You reached the embed limit for this tag"
    })

    if (removeEmbedIndex && (removeEmbedIndex < 1 || removeEmbedIndex > 25)) return int.editReply({
        content: "embed index must be between 1 and 25"
    })

    if (removeEmbedIndex) {
        if (!tagData.data?.embeds || tagData.data.embeds.length === 0) return int.editReply({
            content: "This tag has no embeds",
            ephemeral: true
        })

        if (tagData.data?.embeds?.length < removeEmbedIndex) return int.editReply({
            content: `No embed exists at the index ${removeEmbedIndex}`,
            ephemeral: true
        })

        tagData.data.embeds = tagData.data.embeds.filter((embed, index) => index !== removeEmbedIndex - 1)

        if ((!tagData.data?.content || tagData.data?.content?.length <= 0) && (!tagData.data?.embeds || tagData.data?.embeds <= 0)) return int.editReply({
    ...
    buttonCollector.on("collect", async (inter) => {
        if (inter.customId === "save") {
            ...
            try {
                ...
                return int.editReply({
                    content: "I'm unable to send this embed or to save the database, check the console",
                    ephemeral: true,
                });
            }
        }
