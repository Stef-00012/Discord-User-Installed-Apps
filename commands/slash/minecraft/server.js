...
module.exports = async (client, int) => {
    ...
    switch(serverVersion) {
        case 'java': {
            try {
                const status = await client.functions.getMCJavaServerStatus(serverAddress)
                
                const embed = new EmbedBuilder()
                    .setTitle(`${status.host}${status.port == 25565 ? '' : `:${status.port}`}`)
                    .setAuthor({
        ...
        case 'bedrock': {
            try {
                const status = await client.functions.getMCBedrockServerStatus(serverAddress)
                
                const embed = new EmbedBuilder()
                    .setTitle(`${status.host}${status.port == 19132 ? '' : `:${status.port}`}`)
                    .setAuthor({
