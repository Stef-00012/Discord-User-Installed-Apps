...
export default function (client: Client) {
	...
	router.get(
		...
		async (req: Request, res: Response, next: NextFunction): Promise<any> => {
			if (req.query?.a === "1" || !client.config.web)
				return res.render("auth/notLogged");

			const tokenData: null | DatabaseUserData =
				await client.functions.getToken(client, req.cookies?.id);

			if (tokenData) {
				if (!client.config.owners.includes(tokenData.id))
			...
			const hashedId: string | null = await client.functions.getAccessToken(
				client,
				code as string,
			);
